...
export interface LeadExplorerFilters {
  source?: string;
  planId?: string;
...
export interface MergedLeadItem {
  id: string;
...
export class LeadExplorerQueryService {
  /**
   * Consulta e unifica os leads de acordo com os filtros aplicados
   * combinando a base relacional do DentalGO (MySQL) e o CDP Canônico (Postgres/Prisma).
   */
  static async fetchMatchingLeads(filters: LeadExplorerFilters): Promise<MergedLeadItem[]> {
    const {
      source = 'all',
      planId = 'all',
    ...
    const mysqlLeadMap = new Map<number, any>();
    ...
    if (source === 'all' || source === 'DENTALGO' || planId !== 'all' || subscriptionStatus !== 'all') {
      try {
        ...
        sql += ` ORDER BY p.createdAt DESC LIMIT 5000`;
        ...
        console.warn('[LeadExplorerQueryService] MySQL query bypassed/timed out:', dbErr);
      }
    }
    ...
    if (missingExtIds.length > 0) {
      try {
        ...
        console.warn('[LeadExplorerQueryService] Extra people enrichment failed:', err);
      }
    }

    // 3. Merge MySQL and Prisma leads
    const combinedLeadsMap = new Map<string, MergedLeadItem>();
    ...
    for (const c of prismaCustomers as any[]) {
      ...
      const relationshipTypeComputed = CustomerRelationshipService.classify({
      ...
      combinedLeadsMap.set(key, {
        ...
        relationshipType: relationshipTypeComputed,
        ...
        createdAt: c.createdAt || existing?.createdAt || new Date()
      });
    }
